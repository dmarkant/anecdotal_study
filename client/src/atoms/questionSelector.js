...
export const questionSelector = selector({
  ...
  get: ({ get }) => {
    ...
    switch (questionCondition) {
      ...
      case "share":
        getQL = (tweetText) => {
          return `To what extent would you consider sharing ${tweetText.name}'s post on social media?`;
        };
        return getQL;
        break;
      default:
...
export const qualQuestionSelector = selector({
  ...
  get: ({ get }) => {
    ...
    switch (questionCondition) {
      case "strength":
        getQL = (tweetText) => {
          return `For the post above👆 You made the below👇 judgment when asked: "To what extent does the quoted news headline support ${tweetText.name}'s conclusion?"`;
        };
        return getQL;
      ...
      case "share":
        getQL = (tweetText) => {
          return `For the post above👆 You made the below👇 judgment when asked: "Would you consider sharing ${tweetText.name}'s tweet on social media"`;
        };
        return getQL;
        break;
    }
